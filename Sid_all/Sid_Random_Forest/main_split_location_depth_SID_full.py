...
from functions import load_csv, plot_results, remove_empty_rows
import pandas as pd
import os
...
def main():
    ...
    output_dir = "outputs/SID_LIMNO/figures"
    ...
    columns_to_remove = ['total_dissolved_solids', 'turbidity', 'nitrate', 'posphate', 'ammonium', 'phosphorus']  # Example columns to remove

    # Ensure the dataset is ordered by date in ascending order
    data['date'] = pd.to_datetime(data['date'])  # Convert date column to datetime

    # Define all possible features
    all_features = ['temp', 'ph', 'dissolved_oxygen', 'secchi', 'biochemical_oxygen_demand']
    
    # Remove specified columns from features list
    features = [col for col in all_features if col not in columns_to_remove]
