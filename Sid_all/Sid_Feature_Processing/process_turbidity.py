...
import numpy as np
from process_functions import create_removed_values_matrix
...
def create_removed_values_matrix(df):
    ...
    Returns:
        pd.DataFrame: Matrix with removed values (for validation)
    """
...
def main():
    ...
    results_df.to_csv('output/validation_results.csv', index=False)
    print("\nValidation results saved to: output/validation_results.csv")
