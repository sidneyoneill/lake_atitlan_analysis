...
def save_validation_results(validation_results, output_path="output/validation_results.csv"):
    """
...
def main():
    # Set parameters
    features = ['nitrate', 'phosphate', 'ammonium', 'phosphorus']
    locations = ['SA', 'WG', 'WP']
    cutoff_date = '11/01/2018'
    ...
    # Dictionary to store RMSE values for each location, feature and run
    all_rmse_values = {feature: {loc: [] for loc in locations} for feature in features}
        ...
        for feature in features:
            ...
            for location in locations:
                try:
                    ...
                    all_rmse_values[feature][location].append(rmse)
    ...
    # Calculate statistics for each feature and location
    for feature in features:
        print(f"\n=== {feature.upper()} ===")
        feature_values = []
        
        for location in locations:
            if all_rmse_values[feature][location]:
                mean_rmse = np.mean(all_rmse_values[feature][location])
                std_rmse = np.std(all_rmse_values[feature][location])
                feature_values.extend(all_rmse_values[feature][location])
                
                print(f"\n{location}:")
                print(f"Mean RMSE: {mean_rmse:.4f} ± {std_rmse:.4f}")
                print(f"Individual runs: {', '.join([f'{x:.4f}' for x in all_rmse_values[feature][location]])}")
        
        # Calculate overall statistics for this feature
        if feature_values:
            feature_mean = np.mean(feature_values)
            feature_std = np.std(feature_values)
            print(f"\nOverall {feature} RMSE: {feature_mean:.4f} ± {feature_std:.4f}")
    
    # Save validation results to CSV
    results = {}
    for feature in features:
        for location in locations:
            col_name = f"{feature}_{location}"
            results[col_name] = all_rmse_values[feature][location]
    
    results_df = pd.DataFrame(results)
    results_df.to_csv('output/nutrients_validation_results.csv', index=False)
    print("\nValidation results saved to: output/nutrients_validation_results.csv")
